	...
	for( $i = 1; $i <= $edge_settings['edge_slider_no']; $i++ ){
		...
		if ( !empty( $edge_list_page ) && $edge_total_page_no > 0 ) {
			...
			$image_attributes = wp_get_attachment_image_src($attachment_id,'edge_slider_image');
				...
				if ($image_attributes) {
					//yang benar awal $edge_page_sliders_display 	.= '<div class="image-slider clearfix" title="'.the_title('', '', false).'"' .' style="background-image:url(' ."'" .esc_url($image_attributes[0])."'" .')">';
					//$edge_page_sliders_display 	.= '<a href="'.get_permalink().'" class="image-slider clearfix"  title="'.the_title('', '', false).'"' .'style="background-image:url(' ."'" .esc_url($image_attributes[0])."'" .')"></a>';
					$edge_page_sliders_display 	.= '<div  class="image-slider clearfix" onclick="location.href=\''.get_permalink($recent["ID"]).'\';"  style="cursor: pointer; background-image:url(' ."'" .esc_url($image_attributes[0])."'" .')" >';
				}
				...
				$remove_link = $edge_settings['edge_slider_link'];
					...
					if($edge_settings['edge_slider_button'] == 0){
						if($excerpt_text == '' || $excerpt_text == 'Lihat Produk') :
							/*$edge_page_sliders_display 	.= '<a title='.'"'.get_the_title(). '"'. ' '.'href="'.get_permalink().'"'.' class="btn btn-danger">'.__('Lihat Produk', 'edge').'</a>';*/
							$edge_page_sliders_display 	.= '<a title='.'"'.get_the_title(). '"'. ' '.'href="'.get_permalink().'"'.' class="grey lihatproduk"><i class="fa fa-expand fa-2x" aria-hidden="true"></i></a>';
						else:
